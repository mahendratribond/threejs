...
export function commonMaterialNew(color) {
...
export function getRemoveIcon(removeIconName) {
...
export function setupMainModel(main_model) {
    ...
    main_model.traverse(function (child) {
        if (child.material && baseFrameTextureNames.includes(child.name)) {
            const material = commonMaterial(parseInt(params.baseFrameColor, 16))
    ...
    main_model.traverse(function (modelNode) {
        if (allModelNames.includes(modelNode.name)) {
            modelSize = getModelSize(modelNode.name);
...
export function setupModel(main_model, header_500_height_model, header_wooden_shelf_model, header_rod_model, header_glass_shelf_fixing_model, header_glass_shelf_model, slotted_sides_model, hanger_model, rack_wooden_model) {
    let modelSize, model, header, slotted_left_side, slotted_right_side, frame, hanger, rack;

    if (header_glass_shelf_model) {
        header_glass_shelf_model.traverse(function (child) {
            if (child.material) {
                child.material = generateGlassMaterial(texture_background)
                // child.material = commonMaterial(0xffffff)
                child.material.needsUpdate = true;
            }
        });
    }
    ...
    if (header_rod_model) {
        header_rod_model.traverse(function (child) {
    ...
    if (hanger_model) {
        hanger_model.traverse(function (child) {
            if (child.material && ['Clothing'].includes(child.name)) {
                const material = commonMaterialNew(parseInt(params.defaultClothingColor, 16))
                child.material = material
                child.material.needsUpdate = true;
            }
            if (child.material && ['Hanger_Stand'].includes(child.name)) {
                const material = commonMaterialNew(parseInt(params.defaultHangerStandColor, 16))
                child.material = material
                child.material.needsUpdate = true;
            }
        });
    ...
    if (rack_wooden_model) {
        rack_wooden_model.traverse(function (child) {
            if (child.material && rackNodes.includes(child.name)) {
                const material = commonMaterialNew(parseInt(params.defaultRackColor, 16))
                child.material = material
                child.material.needsUpdate = true;
            }
        });
    ...
    main_model.traverse(function (modelNode) {
        if (allModelNames.includes(modelNode.name)) {
            ...
            // if (rack_wooden_model) {
            //     rack = rack_wooden_model.getObjectByName(modelNode.name);
            //     if (rack) {
            //         rack.name = "Rack";
            //         rack.visible = false;
            //         frame.attach(rack);
            ...
            if (header_glass_shelf_model) {
                ...
                if (model) {
                    ...
                    model.visible = false;
                    model.position.y = glassShelfFixingSize.y / 2;
                    modelNode.attach(model);
                }
            }
...
export function getHeaderSize(value) {
...
export function updateFrameSize(modelGroup, scene, camera) {
    if (modelGroup) {
...
export function showHideNodes(modelGroup, scene, camera) {
    ...
    if (modelGroup) {
        modelGroup.traverse(function (child) {
            ...
            if (child.name === "Header_Wooden_Shelf") {
                child.visible = params.topOption == 'Header_Wooden_Shelf' && params.defaultShelfType == 'Header_Wooden_Shelf';
            }
            if (child.name === "Header_Glass_Shelf") {
                child.visible = params.topOption == 'Header_Wooden_Shelf' && params.defaultShelfType == 'Header_Glass_Shelf';
            }
            if (child.name === "Rod") {
                child.visible = (params.topOption == 'Header_Wooden_Shelf' && ((params.isShelf && params.defaultShelfType == 'Header_Wooden_Shelf') || (params.isGlassShelf && params.defaultShelfType == 'Header_Glass_Shelf'))) || (params.headerRodToggle && params.topOption == 'Header');
            }
            if (child.name === "Glass_Shelf_Fixing") {
                child.visible = params.topOption == 'Header_Wooden_Shelf' && params.isGlassShelf && params.defaultShelfType == 'Header_Glass_Shelf';
            }
            ...
            if (child.material && rodFrameTextureNames.includes(child.name)) {
                child.material.color.set(getHex(params.rodFrameColor));
                child.material.needsUpdate = true;
            }
            if (child.material && ['Hanger_Stand'].includes(child.name)) {
                child.material.color.set(getHex(params.defaultHangerStandColor));
                child.material.needsUpdate = true;
            }
            if (child.material && ['Rack_Wooden_Shelf'].includes(child.name)) {
                child.material.color.set(getHex(params.defaultRackShelfStandColor));
                child.material.needsUpdate = true;
            }
            if (child.material && ['Rack_Stand_LH', 'Rack_Stand_RH'].includes(child.name)) {
...
export function clearMeasurementBoxes(scene) {
...
export function computeBoundingBox(object, frameNames) {
...
export function drawMeasurementBoxesWithLabels(modelGroup, scene, camera) {
    if (modelGroup) {
...
export function addMeasurementGroup(material, lineSize, labelText, handleLineGeometry, groupNamePrefix) {
...
export function updateMeasurementGroups(modelGroup, scene, camera) {
    if (modelGroup) {
        let object = modelGroup.getObjectByName(params.defaultModel);
        if (object) {
            const bbox = computeBoundingBox(object, heightMeasurementNames)
...
export function updateMeasurementGroupPosition(scene, groupName, linePosition, labelPosition, startLinePosition, endLinePosition) {
...
export function generateGlassMaterial(texture_background) {
...
export function isVisibleParents(node) {
...
export function findParentNodeByName(node, parentName) {
