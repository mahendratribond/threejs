...
function init() {
    ...
    const grassTexture = textureLoader.load('assets/images/background/grass-image.jpg', function (texture) {
        ...
        // Calculate repeat values based on ground size and texture size
        // const repeatX = groundGeometry.parameters.width / texture.image.height;
        // const repeatY = groundGeometry.parameters.height / texture.image.width;
    ...
    // Optional: Add a marker to visualize light location (for debugging)
    const ambientLightMarker = createLightMarker(new THREE.Vector3(0, 0, 0), 0xffffff);
    scene.add(ambientLightMarker);
    ...
    // Directional Light
    addDirectionalLight(new THREE.Vector3(30, 60, 0), 0.7);
    addDirectionalLight(new THREE.Vector3(-40, 40, -40), 0.7);
    addDirectionalLight(new THREE.Vector3(40, -28, -40), 0.7);
    addDirectionalLight(new THREE.Vector3(-40, -28, 40), 0.7);

    function addDirectionalLight(position, intensity) {
    ...
    if (borderDropdown) {
        borderDropdown.addEventListener('change', function () {
            ...
            // updateAllFrameBorderTexture(image); // Apply to border parts
        });
...
function generateFilename(key, innerImage, borderImage) {
    ...
    // Alternatively, for MD5 hash version
...
function createLightMarker(position, color) {
