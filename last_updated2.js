...
let renderer, canvas, scene, camera, stats, controls, loader, groupFrame, groupStandFrame, cropper, ambientLight, spotLight1, spotLight2;
let currentBorderColor, currentTopFrame, currentBaseFrame, currentBaseFrameColor, topFramCropedImage, mainFramCropedImage;
let cubeRenderTargetStandFrame, cubeCameraStandFrame;
...
let params = {
    ...
    topFrameShininess: 50,
    baseFrameRoughness: 0.5,
    baseFrameMetalness: 0.01,
    baseFrameReflectivity: 0.9,
    baseFrameClearcoat: 1.0,
    baseFrameClearcoatRoughness: 0.05,

    mainFrameShininess: 50,
    mainFrameRoughness: 0.5,
    mainFrameMetalness: 0.01,
    mainFrameReflectivity: 0.9,
    mainFrameClearcoat: 1.0,
    mainFrameClearcoatRoughness: 0.05,

    topFrameRoughness: 0.5,
    topFrameMetalness: 0.01,
    topFrameReflectivity: 0.9,
    topFrameClearcoat: 1.0,
    topFrameClearcoatRoughness: 0.05,
...
function init() {
    ...
    renderer.setClearColor(0xffffff); // पृष्ठभूमि को सफेद सेट करना
    ...
    groupStandFrame = new THREE.Group();


    camera = new THREE.PerspectiveCamera(50, window.innerWidth / window.innerHeight, 2, 1000);
    camera.position.set(50, 50, 90);

    // Create cube render target for groupStandFrame reflections
    cubeRenderTargetStandFrame = new THREE.WebGLCubeRenderTarget(256, {
        format: THREE.RGBAFormat,
        generateMipmaps: true,
        minFilter: THREE.LinearMipmapLinearFilter
    });

    // Create a cube camera for groupStandFrame reflections
    cubeCameraStandFrame = new THREE.CubeCamera(0.1, 1000, cubeRenderTargetStandFrame);
    scene.add(cubeCameraStandFrame);
    ...
    const groundMaterial = new THREE.MeshPhysicalMaterial({
        ...
        // envMapIntensity: params.groundEnvMapIntensity
    });
    ...
    // Ambient Light
    ambientLight = new THREE.AmbientLight(0xffffff, 1); // Adjust intensity
    ...
    addDirectionalLight(new THREE.Vector3(0, 0, 30), 0.2);
    ...
    // gui.add(params, 'topFrameShininess', 1, 100).name('Top Shininess').onChange(function (value) {
    //     frames.frameTop1.traverse(function (child) {
    //         if (child.isMesh) {
    //             console.log('ggggg')
    //             child.material.shininess = value; // Update ground material metalness
    //         }
    //     });
    // });

    // // Add the groundMetalness controller
    // gui.add(params, 'mainFrameShininess', 1, 100).name('Main Shininess').onChange(function (value) {
    ...
    // // Add the groundMetalness controller
    // gui.add(params, 'topFrameMetalness', 0.0, 1.0).name('Top Metalness').onChange(function (value) {
    //     frames.frameTop1.traverse(function (child) {
    //         if (child.isMesh) {
    //             console.log('fff')
    //             child.material.metalness = value; // Update ground material metalness
    //         }
    //     });
    // });

    // // Add the groundRoughness controller
    // gui.add(params, 'topFrameRoughness', 0.0, 1.0).name('Top Roughness').onChange(function (value) {
    //     frames.frameTop1.traverse(function (child) {
    //         if (child.isMesh) {
    //             console.log('ggggg')
    //             child.material.roughness = value; // Update ground material metalness
    //         }
    //     });
    // });

    // // Add the groundMetalness controller
    // gui.add(params, 'mainFrameMetalness', 0.0, 1.0).name('Main Metalness').onChange(function (value) {
    //     frames.frameMain.traverse(function (child) {
    //         if (child.isMesh) {
    //             console.log(child.material.type);
    //             child.material.metalness = value; // Update ground material metalness
    ...
    // // Add the groundRoughness controller
    // gui.add(params, 'mainFrameMetalness', 0.0, 1.0).name('Main Roughness').onChange(function (value) {
    //     frames.frameMain.traverse(function (child) {
    //         if (child.isMesh) {
    //             console.log(child.material.type);
    //             child.material.roughness = value; // Update ground material metalness
    ...
    //     groundMesh.material.intensity = value; // Adjust ground intensity
    //     groundMesh.material.needsUpdate = true;
    // });
    // // Add the groundIntensity controller
    // gui.add(params, 'groundEnvMapIntensity', 0.0, 5.0).name('Ground Env Map Intensity').onChange(function (value) {
    //     groundMesh.material.envMapIntensity = value; // Adjust ground intensity
    //     groundMesh.material.needsUpdate = true;
    // });
    ...
    if (borderDropdown) {
        borderDropdown.addEventListener('change', function () {
            ...
            Object.keys(singleStandDaeFiles).forEach((key) => {
                ...
                item.fileName = generateFilename(key, defaultFrameSize, innerTexture, borderTexture);
            });
            loadModels()
        });
    }
    ...
    if (frameColorDropdown) {
        frameColorDropdown.addEventListener('change', event => {
            ...
            updateBaseFrameColor(currentBaseFrame, currentBaseFrameColor, true);
        });
...
function generateFilename(key, innerImage, borderImage) {
    return `${key}_${defaultFrameSize}_${getFilenameWithDelimiter(innerImage)}_${getFilenameWithDelimiter(borderImage)}.dae`;
}
...
function updateBaseFrameColor(frame, color, custom = false) {
    let intensity = 0.7;
    let bgColor = '#f1f1f1';
    ...
    if (custom) {
        params.baseFrameMetalness = custom_metalness
        params.baseFrameRoughness = custom_roughness
    }

    // console.log(groupStandFrame)
    // console.log(groupFrame)
    ...
    frame.traverse(mesh => {
        if (mesh.isMesh) {
            ...
            mesh.material.reflectivity = params.baseFrameReflectivity;
            mesh.material.clearcoat = params.baseFrameClearcoat;
            mesh.material.clearcoatRoughness = params.baseFrameClearcoatRoughness;
            mesh.material.roughness = params.baseFrameRoughness;
            mesh.material.metalness = params.baseFrameMetalness;
            // mesh.material.envMap = cubeRenderTargetStandFrame.texture;
    ...
    //     color: 0xffffff,  // Adjust to your desired color
    //     roughness: 0.3,   // Adjust as needed
    //     metalness: 1.0,   // Metalness makes the material reflective
    //     reflectivity: 1.0,
    //     envMap: cubeRenderTargetStandFrame.texture  // Use cube render target for reflections
    // });

    // // Apply this material to all objects within groupStandFrame
    // groupStandFrame.traverse((child) => {
    //     if (child.isMesh) {
    //         child.material = standFrameMaterial;
    //     }
    // });
    // // Update the cube camera's position to match the groupStandFrame's position
    // cubeCameraStandFrame.position.copy(groupStandFrame.position);

    // // Update the cube camera to render reflections around groupStandFrame
    // cubeCameraStandFrame.update(renderer, scene);
...
function checkAndAddToGroup() {
    if (frames.frameBase1 && frames.frameBase2 && frames.frameMain && frames.frameTop1) {
        ...
        console.log('currentBaseFrame', currentBaseFrame)
        // frames.frameMain.position.y = currentBaseFrame.userData.boundingBox.min.y - .6;
        groupStandFrame.add(frames.frameMain);
...
function addLights() {
    if (lightAdded) {
        ...
        for (let i = 0; i < 4; i++) {
            const angle = (i / 4) * Math.PI * 2; // Angle for each light
            const x = Math.cos(angle) * radius;
            const z = Math.sin(angle) * radius;

            const directionalLight = new THREE.DirectionalLight(
                0xffffff,
                i % 2 == 0 ? lightIntensity : 1
            );
            ...
            // Create a new Object3D to act as the light's target
            const targetObject = new THREE.Object3D();
            targetObject.position.copy(groupFrame.position); // Set target to the position of the elf model
...
function switchTopFrame(topFrame) {
    ...
    // currentTopFrame.position.y = mainBox.max.y - 70.495;
    groupStandFrame.add(currentTopFrame);
...
function setBorderColor_new(mesh, bgColor) {
    if (mesh.isMesh && allFrameBorderNames.includes(mesh.name)) {
        const textureLoader = new THREE.TextureLoader();

        var texture = textureLoader.load("./assets/images/borders/Light_Wood.jpg")
        ...
        const shinyMaterialWithTexture = new THREE.MeshStandardMaterial({
            map: texture,
            metalness: 1,
            roughness: 0.2,
            opacity: 1,
        ...
        console.log('mesh UVs:', mesh.geometry.attributes.uv);
        console.log('texture', shinyMaterialWithTexture)

        // Apply the new material to the mesh
        mesh.material = shinyMaterialWithTexture;
        mesh.material.needsUpdate = true;
...
function setBorderColor_working(mesh, bgColor) {
    if (mesh.isMesh) {
        if (allFrameBorderNames.includes(mesh.name)) {
            const textureLoader = new THREE.TextureLoader();

            const texture = textureLoader.load(
                "./assets/images/borders/art.jpg",
                function () {
                    texture.wrapS = THREE.ClampToEdgeWrapping;
                    texture.wrapT = THREE.ClampToEdgeWrapping;
                    texture.encoding = THREE.sRGBEncoding;
            ...
            const shinyMaterialWithTexture = new THREE.MeshStandardMaterial(
                {
                    map: texture, // Apply the texture image
                    metalness: 1, // Full metallic look
                    roughness: 0.2, // Slight roughness for a shiny appearance
                    color: 0xffffff,
                    opacity: 1,
                    transparent: false, // Ensure no color tinting
                }
            );
