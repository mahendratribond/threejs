...
$defaultFrameSize = !empty($_GET['defaultFrameSize']) ? $_GET['defaultFrameSize'] : 600;
foreach ($singleStandDaeFiles as $key => $singleStandDaeFile) {
    $singleStandDaeFiles[$key]['fileName'] = generateFilename($key, $defaultFrameSize, $innerTexture, $borderTexture); // Using MD5 hash for unique file names
}
...
<head>
    ...
    <link href="last_updated2.css" rel="stylesheet">
...
<body>
    ...
    <div id="info" class="fs-5">
        ...
        <table class="table table-sm table-borderless">
            ...
            <tr>
                <th class=""><label>Frame Size</label></th>
                <td class="">
                    <select id="frameSizeDropdown" class="form-select form-select-sm applyTextures">
                        <option value="">Select Frame Size</option>
                        <?php foreach ($frameSizes as $frameSize) {
                            $selected = ($defaultFrameSize == $frameSize) ? 'selected' : '';
                        ?>
                            <option value="<?= $frameSize ?>" <?= $selected ?>><?= $frameSize ?></option>";
                        <?php
                        }
                        ?>
                    </select>
            ...
            <tr>
                <th class=""><label>Frame Color</label></th>
                <td class="">
                    <select id="borderColorDropdown" class="form-select form-select-sm applyTextures">
                        <option value="">Select Frame Color</option>
                        <?php foreach ($borderColors as $color => $colorName) {
                            $selected = ($borderColor == $color) ? 'selected' : '';
                        ?>
                            <option value="<?= $color ?>" <?= $selected ?>><?= $colorName ?></option>";
                        <?php
                        }
                        ?>
                    </select>
            ...
            <tr>
                ...
                <td class="">
                    <select id="frameColorDropdown" class="form-select form-select-sm">
                        ...
                        <?php foreach ($borderColors as $color => $colorName) {
                            $selected = ($defaultBaseFrameColor == $color) ? 'selected' : '';
                        ?>
                            <option value="<?= $color ?>" <?= $selected ?>><?= $colorName ?></option>";
                        <?php
                        }
...
<!-- "three": "./three/build/three.webgpu.js", -->
    ...
    <script>
        ...
        let defaultFrameSize = "<?php echo !empty($defaultFrameSize) ? $defaultFrameSize : ''; ?>";
        ...
        let modelFileExtension = "<?php echo !empty($modelFileExtension) ? $modelFileExtension : ''; ?>";
    </script>
    <script type="module" src="./last_updated2.js"></script>
</body>
