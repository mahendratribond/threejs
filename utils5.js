...
export async function setupGlassRackModel(rack_glass_model, texture_background) {
    if (rack_glass_model) {
...
export async function setupHangerGolfClubModel(hanger_golf_club_model) {
    if (hanger_golf_club_model) {
        ...
        hanger_golf_club_model = await updateModelName(hanger_golf_club_model, 'Hanger_Stand-Fixture Material', 'Hanger_Stand-Fixture_Material')
...
export async function traverseAsync(modelNode, callback) {
    ...
    const promises = modelNode.children.map(child => traverseAsync(child, callback));
...
export async function showHideNodes(main_model, scene, camera) {
    ...
    if (main_model) {
        ...
        await traverseAsync(main_model, async (child) => {
            ...
            if (child.name === "Header_Wooden_Shelf") {
                child.visible = current_setting.topOption == 'Shelf' && isShelf && current_setting.defaultShelfType == 'Header_Wooden_Shelf';
            }
            if (child.name === "Header_Glass_Shelf") {
                child.visible = current_setting.topOption == 'Shelf' && isGlassShelf && current_setting.defaultShelfType == 'Header_Glass_Shelf';
            }
            if (child.name === "Rod") {
                child.visible = (current_setting.topOption == 'Shelf' && ((isShelf && current_setting.defaultShelfType == 'Header_Wooden_Shelf') || (isGlassShelf && current_setting.defaultShelfType == 'Header_Glass_Shelf'))) || (current_setting.headerRodToggle && current_setting.topOption == 'Header');
            }
            ...
            if (headerNames.includes(child.name)) {
                child.visible = current_setting.topOption == 'Header' && current_setting.defaultHeaderSize == child.name;
            }
            ...
            if (child.material && ['Hanger_Stand', 'Hanger_Stand-Arm_Metal', 'Hanger_Stand-Fixture_Material'].includes(child.name) && child.material.color) {
                child.material = child.material.clone();
                child.material.color.set(await getHex(current_setting.defaultHangerStandColor));
                child.material.needsUpdate = true;
            }
            if (child.material && ['Rack_Wooden_Shelf'].includes(child.name) && child.material.color) {
                child.material = child.material.clone();
                child.material.color.set(await getHex(current_setting.defaultRackShelfStandColor));
                child.material.needsUpdate = true;
            }
            if (['Rack_Stand_LH', 'Rack_Stand_RH'].includes(child.name)) {
                if (child.material) {
                    child.material = child.material.clone();
                    child.material.color.set(await getHex(current_setting.defaultRackStandStandColor));
                    child.material.needsUpdate = true;
                }
                else {
                    child.traverse(async function (mesh) {
                        if (mesh.material) {
                            ...
                            mesh.material.color.set(await getHex(current_setting.defaultRackStandStandColor));
                            mesh.material.needsUpdate = true;
                        }
                    });
                }
            ...
            await traverseAsync(main_model, async (modelNode) => {
                if (allModelNames.includes(modelNode.name)) {
                    ...
                    await Promise.all(headerNames.map(async (headerName) => {
                        ...
                        if (header) {
                            if (current_setting.headerRodToggle && !current_setting.headerUpDown) {
                                header.position.y += params.rodSize.y;
                            } else if (!current_setting.headerRodToggle && current_setting.headerUpDown) {
                                header.position.y -= params.rodSize.y;
                            }
                        }
                    }));
    ...
    if (parentElement) {
        ...
        let headerRodColorDropdown = parentElement.querySelector('.headerRodColorDropdown')
        ...
        let headerFrameColorInput = parentElement.querySelector('.headerFrameColorInput')
        if (headerFrameColorInput) {
            headerFrameColorInput.value = await getHex(current_setting.topFrameBackgroundColor)
        }
        let headerFrameColorDropdown = parentElement.querySelector('.headerFrameColorDropdown')
        if (headerFrameColorDropdown) {
            headerFrameColorDropdown.value = current_setting.topFrameBackgroundColor
        }
        let mainFrameColorInput = parentElement.querySelector('.mainFrameColorInput')
        if (mainFrameColorInput) {
            mainFrameColorInput.value = await getHex(current_setting.mainFrameBackgroundColor)
        }
        let baseSelectorDropdown = parentElement.querySelector('.baseSelectorDropdown')
        if (baseSelectorDropdown) {
            baseSelectorDropdown.value = current_setting.selectedBaseFrame
        }
        let hangerClothesToggle = parentElement.querySelector('.hangerClothesToggle')
        if (hangerClothesToggle) {
            hangerClothesToggle.value = current_setting.hangerClothesToggle
        }

        let hangerGolfClubsToggle = parentElement.querySelector('.hangerGolfClubsToggle')
        ...
        if ((current_setting.topOption == 'Header' && current_setting.headerRodToggle) || (current_setting.topOption == 'Shelf' && ((current_setting.defaultShelfType == 'Header_Wooden_Shelf') || (current_setting.defaultShelfType == 'Header_Glass_Shelf')))) {
            parentElement.querySelectorAll('.headerRodColorDropdownBox').forEach(element => {
                element.style.display = 'block';
            });
        }
        else {
        ...
        if (current_setting.topOption == 'Shelf' && ((current_setting.defaultShelfType == 'Header_Wooden_Shelf'))) {
        ...
        if (current_setting.topOption == 'Header' && current_setting.headerOptions == 'SEG') {
            parentElement.querySelectorAll('.headerFrameColorDropdownBox').forEach(element => {
                element.style.display = 'none';
            });
            parentElement.querySelectorAll('.headerFrameColorInputBox').forEach(element => {
                element.style.display = 'block';
            });
        }
        else if (current_setting.topOption == 'Header' && current_setting.headerOptions == 'ALG') {
            parentElement.querySelectorAll('.headerFrameColorDropdownBox').forEach(element => {
                element.style.display = 'block';
            });
            parentElement.querySelectorAll('.headerFrameColorInputBox').forEach(element => {
        ...
        else if (current_setting.topOption == 'Header' && current_setting.headerOptions == 'ALG3D') {
            parentElement.querySelectorAll('.headerFrameColorDropdownBox').forEach(element => {
                element.style.display = 'none';
            });
            parentElement.querySelectorAll('.headerFrameColorInputBox').forEach(element => {
...
export async function drawMeasurementBoxesWithLabels(main_model, scene, camera) {
    if (main_model) {
        const bbox = await computeVisibleNodeBoundingBox(main_model, allModelNames, heightMeasurementNames)
        if (bbox) {
...
// Initialize the label renderer
export async function updateLabelOcclusion(scene, camera, raycaster, direction) {
    ...
    scene.traverse((object) => {
        if (object instanceof CSS2DObject && object.name.includes("Label")) {
            ...
            if (intersects.length > 0 && intersects[0].distance < labelPosition.distanceTo(camera.position)) {
...
export async function createLabel1(text) {
    ...
    const backgroundGeometry = new THREE.ExtrudeGeometry(roundedRectShape, extrudesetting);
    ...
    if (main_model) {
        ...
        if (bbox) {
            ...
            await updateMeasurementGroupPosition(
                ...
                new THREE.Vector3(min.x + width / 2, max.y + 1 + params.measurementLineDistance + lableYAdjustment, cameraZAdjustment + lableZAdjustment), // labelPosition
            ...
            await updateMeasurementGroupPosition(
                ...
                new THREE.Vector3(depthXPosition, min.y + lableYAdjustment, min.z + depth / 2 + lableZAdjustment), // labelPosition
...
export async function updateMeasurementGroupPosition(scene, camera, groupName, linePosition, labelPosition, startLinePosition, endLinePosition) {
...
export async function addAnotherModels(main_model, mergedArray, cameraOnLeft, scene, camera) {
    ...
    await mergedArray.forEach(async (modelName) => {
        if (modelName.startsWith('Other_') && !document.querySelector(`.accordion-item[data-model="${modelName}"]`)) {
            ...
            if (accordionItem) { // Ensure accordionItem is valid
...
export async function cloneAccordionItem(modelName) {
    ...
    newAccordionItem.querySelector('.accordion-header button').textContent = modelName.replace("Other_", ''); // Change the title

    // Set the new ID and aria-controls to ensure uniqueness
    const newId = `collapse${modelName}`;
    newAccordionItem.querySelector('.accordion-collapse').setAttribute('id', newId);
    newAccordionItem.querySelector('.accordion-header button').setAttribute('data-bs-target', `#${newId}`);
    newAccordionItem.querySelector('.accordion-collapse').classList.remove('show'); // Make it collapsed
...
export async function setModelOpacity(main_model, selectedModel, opacityForOthers = 0.5) {
    main_model.traverse((child) => {
        if (child.isGroup) {
