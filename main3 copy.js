...
async function loadModel(model_name) {
    let model_collada = await colladaLoader.loadAsync(model_name);
    let model = model_collada.scene;
...
async function updateMaterial(value, dropdownType) {
    ...
    modelGroup.traverse(async function (child) {
...
if (headerSizeDropdown) {
    ...
    headerSizeDropdown.addEventListener("change", async function (event) {
        ...
        if (!header_500_height_model) {
            header_500_height_model = await loadModel('header_500_height_model.dae');
...
if (topDropdown) {
    ...
    topDropdown.addEventListener("change", async function (event) {
        ...
        await loaderShowHide(true)
        if (!header_rod_model) {
            header_rod_model = await loadModel('header_rod_model.dae');
            params.rodSize = await getNodeSize(header_rod_model)
        }
        if (!header_glass_shelf_fixing_model) {
            header_glass_shelf_fixing_model = await loadModel('header_glass_shelf_fixing_model.dae');
            console.log('header_glass_shelf_fixing_model', header_glass_shelf_fixing_model)
            params.glassShelfFixingSize = await getNodeSize(header_glass_shelf_fixing_model);
            await setupGlassShelfFixingModel(modelGroup, header_rod_model, header_glass_shelf_fixing_model);
        }
        if (!header_500_height_model) {
            header_500_height_model = await loadModel('header_500_height_model.dae');
            await setupHeader500HeightModel(modelGroup, header_500_height_model);
        ...
        if (!header_wooden_shelf_model) {
            header_wooden_shelf_model = await loadModel('header_wooden_shelf_model.dae');
            await setupHeaderWoodenShelfModel(modelGroup, header_wooden_shelf_model);
            await updateMaterial(params.defaultShelfColor, 'shelf');
        }
        if (!header_glass_shelf_model) {
            header_glass_shelf_model = await loadModel('header_glass_shelf_model.dae');
            await setupHeaderGlassShelfModel(modelGroup, header_glass_shelf_model, texture_background);
        }
...
if (slottedSidesToggle) {
    ...
    slottedSidesToggle.addEventListener("change", async function (event) {
        ...
        if (!slotted_sides_model) {
            slotted_sides_model = await loadModel('slotted_sides_model.dae');
...
if (addHanger) {
    addHanger.forEach(button => {
        button.addEventListener('click', async function () {
            await loaderShowHide(true)
            if (!hanger_model) {
                hanger_model = await loadModel('hanger_model.dae');
                await setupHangerModel(modelGroup, hanger_model);
            }
    ...
    addRack.forEach(button => {
        button.addEventListener('click', async function () {
            ...
            if (!rack_wooden_model) {
                rack_wooden_model = await loadModel('rack_wooden_model.dae');
                await setupWoodenRackModel(modelGroup, rack_wooden_model);
            }
            if (!rack_glass_model) {
                rack_glass_model = await loadModel('rack_glass_model.dae');
                await setupGlassRackModel(modelGroup, rack_glass_model, texture_background);
            }
