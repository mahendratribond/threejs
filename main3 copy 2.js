...
const GLTFLoaderaLoader = new GLTFLoader(manager).setPath("./assets/models/");
...
const lightHelpers = [];
const hangerArray = [];
const shadow = await commonMaterial(0x444444)
...
window.addEventListener('load', async () => {
    try {
        ...
        main_model_collada = await colladaLoader.loadAsync(params.defaultModel + '.dae');
...
async function init() {
    ...
    if (main_model_collada) {
        ...
        //     let model_name = val + '.dae';
        //     let already_added = main_model.getObjectByName(val);
        //     if (!already_added) {
        //         let model_load = await loadModel(colladaLoader, model_name);
...
async function onMouseMove(event) {
    ...
    // console.log('hangerNames', hangerNames);
    ...
    if (hangerIntersects.length > 0) {
        ...
        if (intersectNode) {
            ...
            if (iconName.startsWith('removeHanger-')) {
                ...
                if (hangerNames.includes(selectedNode.name)) {
                ...
                else if (rackNames.includes(selectedNode.name)) {
                    // Attach transform controls to the selected node
                    transformControls.attach(selectedNode);
                    transformControls.setMode('translate');  // Set the mode to 'translate' for moving
                    ...
                    let selectedParentNode = await findParentNodeByName(selectedNode, "Iron_Arm")
                    ...
                    selectedParentNode = await findParentNodeByName(selectedNode, "Driver_Arn")
                    if (selectedParentNode) {
                        selectedNode = selectedParentNode
                        // Attach transform controls to the selected node
                        transformControls.attach(selectedNode);
...
async function enforceHangerBounds() {
    if (selectedNode) {
        ...
        if(!selectedChildNode){
            selectedChildNode = selectedNode.getObjectByName("Driver_Arm");
        }
        if(!selectedChildNode){
            selectedChildNode = selectedNode.getObjectByName("Iron_Arm_1");
        }
...
// Function to enforce boundaries on X-axis
async function enforceRackBounds2() {
    if (selectedNode) {
    ...
    currentModel.traverse(function (modelNode) {
        ...
        if (frame) {
            frame.traverse(async function (child) {
                await updateUploadedTexture(child, texture, frameMainNames)
            });
        }
    ...
    currentModel.traverse(function (modelNode) {
        ...
        if (header) {
            header.traverse(async function (child) {
                await updateUploadedTexture(child, texture, frameTop1Names)
            });
        }
...
if (topFrameFileUpload) {
    topFrameFileUpload.addEventListener("change", async function (event) {
        ...
        reader.onload = async function (e) {
            ...
            const size = await getCurrentModelSize(currentHeader, 'Header_Graphic.1');
...
if (mainFrameFileUpload) {
    mainFrameFileUpload.addEventListener("change", async function (event) {
        ...
        reader.onload = async function (e) {
            ...
            let currentModel = scene.getObjectByName(params.defaultModel);
            const size = await getCurrentModelSize(currentModel, 'Cube.1');
            // console.log(size)
...
if (addHanger) {
    addHanger.forEach(button => {
        button.addEventListener('click', async function () {
            ...
            if (['Iron_Arm', 'Driver_Arn'].includes(hangerType)) {
                // if (!henger_golf_club_model) {
                //     henger_golf_club_model = await loadModel(colladaLoader, 'henger_golf_club_model.dae');
                //     await setupHangerGolfClubModel(main_model, henger_golf_club_model);
                // }
                // hangermodel = henger_golf_club_model;

                let frame = defaultModel.getObjectByName('Frame');
                const loader = new GLTFLoader();
                golf_club = await loadGLTFModel(loader, "./assets/models/golf_club.glb");
                // golf_club.name = 'golf_club'
                hangermodel = golf_club;


                // frame.add(golf_club);
                //     golf_club.position.set(0, -107.5, 0);
                console.log('golf_club', golf_club)

                // // let texture_border = new THREE.TextureLoader().load("./assets/images/borders/Light_Wood.jpg");
                // let texture_border = new THREE.TextureLoader().load("./assets/images/inner-images/Logo_Test.jpg");
                // texture_border = await setTextureParams(texture_border);
                // texture_border.flipY = false;

                // golf_club.traverse(async function (modelNode) {
                //     if (['Iron_Arm-Arm_Metal'].includes(modelNode.name)) {
                //         // if (['Faceplate-Logo'].includes(modelNode.name) && modelNode.parent && modelNode.parent.name == 'Faceplate') {
                //         console.log('modelNode.name', modelNode.name)
                //         // modelNode.material = await commonMaterial(parseInt(params.baseFrameColor, 16))
                ...
                if (hanger_object) {
                    ...
                    if (hanger) {
                        ...
                        if ((!leftSideSlotted || !leftSideSlotted.visible) && conditionFlag) {
                            ...
                            hanger.position.x = hanger.position.x / 10;
                            hanger.position.y = hanger.position.y / 10;
                            hanger.position.z = hanger.position.z / 10;

                            hanger.position.y -= params.cameraPosition * 1.34;
                            hanger.position.z -= 3.5;
                            hanger.position.x = 0;

                            hanger.scale.set(0.1, 0.1, 0.1);
                            ...
                            // removeHangerIcon.scale.set(1, 1, 1);

                            // Adjust the hanger position based on the camera's z-axis position
                            if (side == "Front") {
                                hanger.rotation.y = Math.PI;
                                if (['Iron_Arm', 'Driver_Arn'].includes(hangerType)) {
                                    hanger.position.z = frame.position.z - hangerBoundingBox.max.z - 4; // Small offset in front of the frame
                                }
                                else {
