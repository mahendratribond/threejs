...
const headerRodColorDropdown = document.querySelector(".headerRodColorDropdown");
...
const headerFrameColorDropdown = document.querySelector(".headerFrameColorDropdown");
...
let gui, stats;
let renderer, scene, camera, controls, transformControls, raycaster, mouse, direction, hangerIntersects, selectedNode, labelRenderer;
    ...
    main_model.traverse(async function (modelNode) {
        if (allModelNames.includes(modelNode.name)) {
            modelNode.traverse(async function (child) {
                if (frameTop1Names.includes(child.name) || frameMainNames.includes(child.name)) {
...
async function otherModelSetup() {
    ...
    if (!header_glass_shelf_fixing_model) {
        header_glass_shelf_fixing_model = await loadGLTFModel(glftLoader, 'header_glass_shelf_fixing_model.glb');
    ...
    if (!header_500_height_model) {
        header_500_height_model = await loadGLTFModel(glftLoader, 'header_500_height_model.glb');
    ...
    if (!header_wooden_shelf_model) {
        header_wooden_shelf_model = await loadGLTFModel(glftLoader, 'header_wooden_shelf_model.glb');
    ...
    if (!header_glass_shelf_model) {
        header_glass_shelf_model = await loadGLTFModel(glftLoader, 'header_glass_shelf_model.glb');
    ...
    if (!slotted_sides_model) {
        slotted_sides_model = await loadGLTFModel(glftLoader, 'slotted_sides_model.glb');
        ...
        if (intersectNode) {
            ...
            if (iconName.startsWith('removeHanger-')) {
                ...
                if (selectedHangerNode) {
                    ...
                    let removeHanger = selectedNode.getObjectByName('removeHanger-' + selectedNode.name);
                    if (removeHanger) {
                ...
                if (!selectedRackNode && rackNames.includes(selectedNode.name) && await isVisibleParents(selectedNode)) {
                    selectedRackNode = selectedNode
                }
                if (selectedRackNode) {
                    selectedNode = selectedRackNode
                    let removeRack = selectedNode.getObjectByName('removeRack-' + selectedNode.name);
                    if (removeRack) {
    ...
    if (selectedNode) {
        ...
        if (defaultModel) {
            ...
            if (position.x < adjustedMinX) {
                ...
                position.x = THREE.MathUtils.clamp(position.x, adjustedMinX, adjustedMaxX);
            }
...
async function setMainFrameCropedImage() {
    ...
    if (mainFramCropedImage[defaultModelName]) {
        const mainFrameBackgroundColor = await getHex(setting[params.selectedModelName].mainFrameBackgroundColor);
...
async function setTopFrameCropedImage() {
    ...
    if (topFramCropedImage[defaultModelName][defaultHeaderSize]) {
        const topFrameBackgroundColor = await getHex(setting[params.selectedModelName].topFrameBackgroundColor);
        const tempCanvas = document.createElement('canvas');
        tempCanvas.width = topFramCropedImage[defaultModelName][defaultHeaderSize].width;
        tempCanvas.height = topFramCropedImage[defaultModelName][defaultHeaderSize].height;
        const ctx = tempCanvas.getContext('2d');
        ...
        // Draw the cropped image on top
        ctx.drawImage(topFramCropedImage[defaultModelName][defaultHeaderSize], 0, 0);
...
async function updateMaterial(value, dropdownType, selectedModel = 'default') {
    ...
    const customDropdownButton = document.querySelector(`.accordion-item[data-model=${selectedModel}] .custom-dropdown[data-type=${dropdownType}]`);
    ...
    const dropdownButton = customDropdownButton.querySelector(`.accordion-item[data-model=${selectedModel}] .dropdown-button`);
    if (dropdownButton) {
...
if (topDropdown) {
    ...
    document.addEventListener('change', async function (event) {
        if (event.target.classList.contains('topDropdown')) {
            console.log('setting[params.selectedModelName]', setting[params.selectedModelName])
            ...
            if (setting[params.selectedModelName].topOption == 'Header_Wooden_Shelf') {
                setting[params.selectedModelName].headerRodToggle = true;
            }

            headerRodToggle.checked = setting[params.selectedModelName].headerRodToggle;
            await loaderShowHide(true)
            await otherModelSetup()
            await loaderShowHide(false)
            await showHideNodes(main_model, scene, camera);
...
if (slottedSidesToggle) {
    ...
    document.addEventListener('change', async function (event) {
        if (event.target.classList.contains('slottedSidesToggle')) {
            setting[params.selectedModelName].slottedSidesToggle = event.target.checked;
...
if (headerFrameColorInput) {
    ...
    document.addEventListener('input', async function (event) {
        if (event.target.classList.contains('headerFrameColorInput')) {
            setting[params.selectedModelName].topFrameBackgroundColor = event.target.value;
            await loaderShowHide(true)
            await otherModelSetup()
            await loaderShowHide(false)
...
if (mainFrameColorInput) {
    ...
    document.addEventListener('input', async function (event) {
        if (event.target.classList.contains('mainFrameColorInput')) {
            setting[params.selectedModelName].mainFrameBackgroundColor = event.target.value;
...
if (hangerClothesToggle) {
    ...
    document.addEventListener('change', async function (event) {
        if (event.target.classList.contains('hangerClothesToggle')) {
            setting[params.selectedModelName].hangerClothesToggle = event.target.checked;
            await loaderShowHide(true)
            await otherModelSetup()
            await loaderShowHide(false)
            await showHideNodes(main_model, scene, camera)
...
if (hangerGolfClubsToggle) {
    ...
    document.addEventListener('change', async function (event) {
        if (event.target.classList.contains('hangerGolfClubsToggle')) {
            setting[params.selectedModelName].hangerGolfClubsToggle = event.target.checked;
            await loaderShowHide(true)
            await otherModelSetup()
            await loaderShowHide(false)
            await showHideNodes(main_model, scene, camera)
...
if (hangerStandColor) {
    ...
    document.addEventListener('change', async function (event) {
        if (event.target.classList.contains('hangerStandColor')) {
            setting[params.selectedModelName].defaultHangerStandColor = event.target.value;
            await loaderShowHide(true)
            await otherModelSetup()
            await loaderShowHide(false)
            await showHideNodes(main_model, scene, camera)
...
if (rackShelfColor) {
    ...
    document.addEventListener('change', async function (event) {
        if (event.target.classList.contains('rackShelfColor')) {
            setting[params.selectedModelName].defaultRackShelfStandColor = event.target.value;
            await loaderShowHide(true)
            await otherModelSetup()
            await loaderShowHide(false)
            await showHideNodes(main_model, scene, camera)
...
if (rackStandColor) {
    ...
    document.addEventListener('change', async function (event) {
        if (event.target.classList.contains('rackStandColor')) {
            setting[params.selectedModelName].defaultRackStandStandColor = event.target.value;
            await loaderShowHide(true)
            await otherModelSetup()
            await loaderShowHide(false)
            await showHideNodes(main_model, scene, camera)
    ...
    cropButton.addEventListener("click", async function (event) {
        if (cropper) {
            ...
            if (params.fileUploadFlag == 'MainFrame') {
                ...
                topFramCropedImage[defaultModelName] = topFramCropedImage[defaultModelName] || {};
                topFramCropedImage[defaultModelName][defaultHeaderSize] = cropper.getCroppedCanvas();
                await setTopFrameCropedImage()
            }
        }
    });
                    ...
                    if (hanger) {
                        ...
                        if ((!leftSideSlotted || !leftSideSlotted.visible)) {
                            if (conditionFlag) {
                                ...
                                const hangerCenter = hangerBoundingBox.getCenter(new THREE.Vector3());
                                const hangerLength = hangerBoundingBox.max.z - hangerBoundingBox.min.z;
...
if (addRack) {
    ...
    document.addEventListener("click", async function (event) {
        if (event.target.closest(".addRack")) {
            ...
            let defaultModelName = params.selectedModelName !== 'default' ? params.selectedModelName : params.defaultModel;
            let getCleanModelName = params.selectedModelName !== 'default' ? await cleanModelName(defaultModelName) : params.defaultModel;;
            ...
            if (rack_model) {
                ...
                if (rack) {
                    ...
                    if (topExSide && leftSideSlotted && leftSideSlotted.visible) {
                        ...
                        const topExSideCenter = topExSideBoundingBox.getCenter(new THREE.Vector3()); // Get center of frame

                        const boundingBox = params.calculateBoundingBox[getCleanModelName][frame.name];
    ...
    addAnotherModel.forEach(button => {
        button.addEventListener('click', async function () {
            ...
            if (defaultModel) {
                ...
                await addAnotherModels(model, allModelNames, cameraOnLeft, scene, camera);
...
moveLeftModel.addEventListener("click", async () => {
    ...
    if (defaultModel) {
        ...
        const canMoveLeft = await checkForCollision(main_model, defaultModel, -params.moveLeftRight);
...
moveRightModel.addEventListener("click", async () => {
    ...
    if (defaultModel) {
        ...
        const canMoveRight = await checkForCollision(main_model, defaultModel, params.moveLeftRight);
...
accordionModel.addEventListener('show.bs.collapse', async function (event) {
    const openAccordionItems = accordionModel.querySelectorAll('.accordion-collapse.show');
...
document.addEventListener("click", function (event) {
    ...
    if (event.target.closest(".dropdown-button")) {
        ...
        if (dropdownContent) {
            ...
            dropdownContent.querySelectorAll(".custom-dropdown .dropdown-content").forEach(function (content) {
...
window.addEventListener("click", function (event) {
    if (!event.target.closest('.custom-dropdown')) {
        document.querySelectorAll(".custom-dropdown .dropdown-content").forEach(function (content) {
