...
export async function isHangerAdd(hangerType, hangerArray, hangerArrayKey) {
    ...
    if (['Hanger_Rail_Single', 'Hanger_Golf_Club_Iron', 'Hanger_Golf_Club_Driver'].includes(hangerType)) {
    ...
    return new Promise((resolve, reject) => {
        loader.load(
            ...
            function (model_load) {
                ...
                // model.scale.set(0.01, 0.01, 0.01); // Adjust the scale if necessary
                resolve(model);
            },
            undefined,
            function (error) {
...
export async function setupHangerGolfClubModel(main_model, henger_golf_club_model) {
    if (henger_golf_club_model) {
        henger_golf_club_model.traverse(async function (child) {
            if (child.material && ['Hanger_Stand'].includes(child.name)) {
                const material = await commonMaterial(parseInt(params.defaultHangerStandColor, 16))
                child.material = material
                child.material.needsUpdate = true;
...
export async function drawMeasurementBoxesWithLabels(main_model, scene, camera) {
    if (main_model) {
        let object = main_model.getObjectByName(params.defaultModel);
...
export async function findParentNodeByName(node, parentName) {
    ...
    const result = await findParentNodeByName(node.parent, parentName);
