    ...
    addAnotherModel.forEach(button => {
        button.addEventListener('click', async function () {
            ...
            if (params.mainModel && params.mainModel[modelType]) {
                ...
                model.name = 'Other_' + modelType;

                const newModelBoundingBox = new THREE.Box3().setFromObject(model);
                const newModelWidth = newModelBoundingBox.min.x - newModelBoundingBox.max.x
                model.localToWorld(newModelBoundingBox.min);
                model.localToWorld(newModelBoundingBox.max);

                const mergedArray = allModelNames.concat(allOtherModelNames);
                const modelBoundingBox = await computeBoundingBox(main_model, mergedArray);
                const modelWidth = modelBoundingBox.min.x - modelBoundingBox.max.x

                console.log('main_model.position', main_model.position)
                console.log('modelBoundingBox', modelBoundingBox)
                console.log('newModelBoundingBox', newModelBoundingBox)
                console.log('modelWidth', modelWidth)
                console.log('newModelWidth', newModelWidth)

                // model.position.y = +params.cameraPosition //+ boundingBox.max.x
                model.position.x = newModelWidth / 2 + modelWidth / 2  //+ boundingBox.max.x
...
if (addHanger) {
    addHanger.forEach(button => {
        button.addEventListener('click', async function () {
            ...
            if (['Hanger_Golf_Club_Iron', 'Hanger_Golf_Club_Driver'].includes(hangerType)) {
                ...
                // golf_club.scale.set(0.1, 0.1, 0.1); // Adjust the scale if necessary
                // golf_club.position.set(0, -params.cameraPosition, 0);
                // scene.add(golf_club);
